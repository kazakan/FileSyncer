...
class FSServer(var rootPath: File, var port: Int = 5050) : FSMessageBroadcaster<FSEventMessage> {
  ...
  fun initialize() {
    ...
    if (!rootDir.exists()) {
      ...
      if (!rootDir.isDirectory) throw Error("Repo path($rootDir) is already occupied by other file")
    }
          ...
          override fun handleMessage(msg: FSEventMessage) {
            when (msg.mEventcode) {
              ...
              FMEVENT_TYPE.LOGIN_REQUEST -> {
                ...
                if (userManager.userExists(user)) {
                  if (userManager.addUserSession(user, connWorker) == null) {
                    ...
                    println("server : login requested by ${msg.userIdField.str} rejected.")
                    connWorker.putMsgToSendQueue(FSEventMessage(FMEVENT_TYPE.LOGIN_REJECTED))
                  } else {
                    // granted
                    println("server : login requested by ${msg.userIdField.str} granted.")
              ...
              FMEVENT_TYPE.LOGOUT -> {
                ...
                this@FSServer.broadcast(
                    FSEventMessage(
                        FMEVENT_TYPE.BROADCAST_CONNECTED, userIdStr = msg.userIdField.str))
              }
